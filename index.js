...
// --- FINAL 22 CATEGORIES ---
const categories = [
  'tugilgan kun uchun', 'printer rang', 'Printer tuzatish', 
  'remont-tuzatish', 'Hodimlar uchun dorilar', 'jihoz', 
  'Texnikalar', 'Transport', 'aromatizator', 'Internet', 
  'Telefon', 'of the month', 'Event', 'Reklama mahsulotlarini chiqarish', 
  'giftbox sovgalar', 'syomka xarajatlari', 'bozorlik xojalik', 
  'Suv va stakan', 'Konstovar', 'Plastik foizi', 
  'ofis xarajatlari', 'remont qurilish'
];

const showMainMenu = (ctx, text = 'Select a category:') => {
...
bot.command(['start', 'new'], (ctx) => {
  ...
  showMainMenu(ctx, 'IELTS Zone Finance Bot 🎓');
});
...
bot.on('text', async (ctx) => {
  ...
  if (text === '❌ Cancel') {
    ...
    return showMainMenu(ctx, 'Cancelled. New request:');
  }
  ...
  if (session.step === 'CATEGORY') {
    if (categories.includes(text)) {
      ...
      session.step = 'AMOUNT';
      return ctx.reply(`Category: ${text}\n\nHow much? (Numbers only):`, Markup.keyboard(['❌ Cancel']).resize());
    }
  ...
  if (session.step === 'AMOUNT') {
    ...
    if (!amt) return ctx.reply('Please enter only digits.');
    session.amount = amt;
    session.step = 'DESCRIPTION';
    return ctx.reply(`Amount: ${amt} UZS\n\nWhat is this for? (Reason):`);
  }
...
bot.action(/^(app|rej)_(.+)$/, async (ctx) => {
  ...
  try {
    ...
    const amount = targetRow.get('Amount');
